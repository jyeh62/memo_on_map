...
public class SearchDemoActivity extends FragmentActivity implements MapView.MapViewEventListener, MapView.POIItemEventListener {
	...
	private void showResult(List<Item> itemList) {
		...
		for (int i = 0; i < itemList.size(); i++) {
            ...
            android.util.Log.d("SearchDemo" , "showResult = " + item.title + ", lat = " + item.latitude + ", long = " + item.longitude);
	...
	public void onCalloutBalloonOfPOIItemTouched(MapView mapView, MapPOIItem mapPOIItem, MapPOIItem.CalloutBalloonButtonType calloutBalloonButtonType) {
        ...
        Intent intent = new Intent(this, MemoEditorActivity.class).setAction(Intent.ACTION_INSERT).setData(DBConst.MemoColumns.CONTENT_URI);
        intent.putExtra("name", item.title);
        intent.putExtra("tel", item.phone);
        intent.putExtra("lat", Double.toString(item.latitude));
        intent.putExtra("long", Double.toString(item.longitude));
